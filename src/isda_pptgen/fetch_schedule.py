...
SPREADSHEET_ID = "1un6Na8K3DR6b-xXi_npPlgqtTaxS7XdKD5yAYEcFVy0"


def fetch_data_for_date(date_str: str) -> dict | None:
    try:
        ...
        for row in service_records:
            ...
            if row_date == date_str:
                service_data = {
                    ...
                    "Call to worship": get_value(
                        row, "Call to worship", "Call to worship", "Call To Worship"
                    ),
                    "Offerings": get_value(row, "Offerings", "Offerings"),
                    "Song service song 1": get_value(
                        row, "Song service song 1", "Song service song 1"
                    ),
                    "Song service song 2": get_value(
                        row, "Song service song 2", "Song service song 2"
                    ),
        ...
        print(
            "Error: 'credentials.json' not found. Please follow the setup instructions to create a Google Service Account."
        )
        return None
        ...
        traceback.print_exc()
        print(f"Error fetching data: {e!r}")
        return None


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Fetch Worship Service data from Google Sheets"
    )
    parser.add_argument(
        "date",
        help="Date to fetch data for (e.g., '4/18/2026' or '2026-04-18', must exactly match the formatting shown in your Google Sheet's Date column)",
    )
