...
def main():
    ...
    ws_parser = subparsers.add_parser(
        "build-ws", help="Build a worship service presentation"
    )
    ...
    lyrics_parser = subparsers.add_parser(
        "generate-lyrics", help="Generate hymn lyrics presentations"
    )
    lyrics_parser.add_argument(
        "--force", action="store_true", help="Force regenerate all"
    )
    ...
    songs_parser = subparsers.add_parser(
        "songs-manager", help="Start the Streamlit Songs Manager"
    )

    # Command 5: Create config
    create_parser = subparsers.add_parser(
        "create", help="Create an empty .yml file for the upcoming Saturday"
    )
    create_parser.add_argument(
        "-p",
        "--populate",
        action="store_true",
        help="Automatically populate from Google Sheet",
    )

    # Command 6: Populate config
    populate_parser = subparsers.add_parser(
        "populate", help="Populate empty fields in a given config from Google Sheet"
    )
    ...
    images_parser = subparsers.add_parser(
        "images-to-slides",
        help="Generate a presentation from images in the current directory",
    )
    images_parser.add_argument(
        "-o",
        "--output",
        default="images_presentation.pptx",
        help="Output file name (default: images_presentation.pptx)",
    )
    images_parser.add_argument(
        "-c", "--caption", default="", help="Caption to add to each slide"
    )
    images_parser.add_argument(
        "--extensions",
    ...
    if args.command == "build-ws":
        ...
        from isda_pptgen.build_ws import (
            DEFAULT_CONFIG,
            build_presentation,
            load_yaml_config,
        )
        ...
        from isda_pptgen.hymn_lyrics_generator import (
            generate_all_external_songs,
            generate_all_hymns,
        )
